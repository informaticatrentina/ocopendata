...
<div class="container">
    ...
    <script>
    $(function() {ldelim}
        var endpoint = "{'api/opendata/v2/content/search'|ezurl(no)}/";
        ...
        var loadSearchResults = function(data){
            ...
            var content = '<pre>'+data.query+'</pre>';
            content += '<h2>Trovati '+data.totalCount+ ' risultati</h2>';
            if ( results.length > 0 ) {
                content += '<h3>Visualizzati ' + results.length + ' risultati ';
                if ( data.nextPageQuery !== null )
                    content += '<a href="'+data.nextPageQuery+'" class="search">(pagina successiva)</a></h3>';
                else
                    content += '</h3>';
        ...
        $(document).on( 'click', 'a.search', function(e){
            search($(e.currentTarget).attr('href'));
            e.preventDefault();
        });
